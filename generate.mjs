...
;(async()=>{
  ...
  fs.writeFileSync('data/mm-simonly.json',JSON.stringify(simonly))
  fs.writeFileSync('data/mm-devices.json',JSON.stringify(devices))
  const meta={updated:new Date().toISOString(),simonly:simonly.length,devices:devices.length}
  fs.writeFileSync('data/mm-meta.json',JSON.stringify(meta,null,2))
